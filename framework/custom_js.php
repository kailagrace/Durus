...
(function($){
	...
	jQuery(document).ready(function($){
	  var retina = window.devicePixelRatio > 1 ? true : false;
         <?php if($brad_data['media_logo_retina']['url'] && $brad_data['logo_width'] && $brad_data['logo_height']): ?>
        if(retina) {
        	jQuery('#logo img').attr('src', '<?php echo $brad_data["media_logo_retina"]['url']; ?>');
        	jQuery('#logo img').css('width', '<?php echo $brad_data["logo_width"]; ?>');
        	jQuery('#logo img').css('height', '<?php echo $brad_data["logo_height"]; ?>');
        }
        <?php endif; ?>
